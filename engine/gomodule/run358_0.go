...
func main() {
	...
	for t := 0; t < thread_num; t++ {
		for i := 0; i < 172; i++ {
			...
			engine.SetInputData(t, name, features[name], []int32{int32(batch_size), 1})
		}
		for i := 172; i < 344; i++ {
			name := "field_" + strconv.Itoa(i)
			engine.SetInputData(t, name, features_int[name], []int32{int32(batch_size), 1})
		}
		for i := 344; i < 358; i++ {
			name := "field_" + strconv.Itoa(i)
			engine.SetInputData(t, name, features_int[name], []int32{int32(batch_size), 10})
		}
	}
	...
	for i := 0; i < request_num; i++ {
		...
		go func(key_id int) {
			start_time := time.Now()
			engine.Run160(key_id, res0[key_id], res1[key_id], res2[key_id])
