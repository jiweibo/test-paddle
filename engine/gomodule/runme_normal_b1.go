...
func main() {
	...
	batch_size := 1 // int(features["itemSize"][0])
	...
	for i := 0; i < request_num; i++ {
		...
		go func(key_id int) {
			...
			// engine.RunNormalF(key_id, batch_size, features, features_int, res0[key_id], res1[key_id], res2[key_id])
			last := time.Now().Sub(start_time)
			fmt.Println(res2[key_id])
			// print avg output.
			var avg0 float32 = 0.
			var avg1 float32 = 0.
			var avg2 float32 = 0.
...
func read_json_data() (map[string][]float32, map[string][]int64) {
	...
	file_bytes, _ := ioutil.ReadFile("data/input.txt")
