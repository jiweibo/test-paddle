...
func main() {
	...
	go func() {
		...
		for i := 0; i < 10; i++ {
			time.Sleep(d)
			fmt.Println("prepare to reload model")
			reload_id = engine.Reload("models/160inut/", "", "", thread_num, math_num, use_lite, 0)
			fmt.Println("reload model done, reload_id is ", reload_id)
		}
		defer func() {
			wg.Done()
		}()
	...
	for i := 0; i < request_num; i++ {
		...
		go func(key_id int, reload_id int32) {
			...
			for n, v := range features {
				engine.SetInputData(reload_id, key_id, n, v, []int32{int32(batch_size), int32(1)})
			}
